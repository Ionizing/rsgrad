...
struct Configuration {
    ...
    //#[serde(default = "Configuration::xlim_default")]
    //xlim: (f64, f64),
...
pub struct Dos {
    ...
    //#[clap(long, default_values = &["-1", "6"], number_of_values = 2)]
...
const TEMPLATE: &str = include_str!("./pdos_template.toml");
...
impl OptProcess for Dos {
    fn process(&self) -> Result<()> {
        ...
        let procar     = config.as_ref().map(|cfg| &cfg.procar).unwrap_or(&self.procar);
        let outcar     = config.as_ref().map(|cfg| &cfg.outcar).unwrap_or(&self.outcar);
        let txtout     = config.as_ref().map(|cfg| &cfg.txtout).unwrap_or(&self.txtout);
        let htmlout    = config.as_ref().map(|cfg| &cfg.htmlout).unwrap_or(&self.htmlout);
        let sigma      = config.as_ref().map(|cfg| cfg.sigma).unwrap_or(0.05);
        ...
        //let xlim       = config.as_ref().map(|cfg| vec![cfg.xlim.0, cfg.xlim.1]).unwrap_or(self.xlim.clone().unwrap());
